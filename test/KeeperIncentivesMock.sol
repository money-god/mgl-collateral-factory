...
abstract contract BaseFeeIncentive {
    ...
    modifier payRewards() {
        ...
        if (block.timestamp >= lastCallMade + callDelay) {
            gas = gas - gasleft();
            uint256 coinCost = (gas * 35 * 10 ** 9 * ethOracle.read());
            coinOracle.read();
...
abstract contract OSMLike {
    ...
    function getNextResultWithValidity()
        external
        view
        virtual
...
contract BasefeeOSMDeviationCallBundler is BaseFeeIncentive {
    ...
    ) public BaseFeeIncentive(treasury_, reward_, delay_, coinOracle_, ethOracle_) {
    ...
    fallback() external payRewards {
        ...
        if (collateralA != bytes32(0))
            oracleRelayer.updateCollateralPrice(collateralA);
        if (collateralB != bytes32(0))
            oracleRelayer.updateCollateralPrice(collateralB);
        if (collateralC != bytes32(0))
            oracleRelayer.updateCollateralPrice(collateralC);
    }
}
...
contract KeeperIncentivesFactory {
    function deploy(
        ...
        address osm_,
        address oracleRelayer_,
        bytes32[3] calldata collateral_,
        uint256 reward_,
        uint256 delay_,
        address coinOracle_,
        address ethOracle_,
